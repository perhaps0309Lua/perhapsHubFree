...
local ProtectGui = protectgui or (gethui and function(v) v.Parent = gethui() end) or (syn and syn.protect_gui) or (function() end);
...
function addSignal(Signal: RBXScriptConnection)
...
function Library:MakeDraggable(Instance, Cutoff)
   ...
   local inputBegan = Instance.InputBegan:Connect(function(Input)
...
do
    ...
    function Funcs:AddColorPicker(Idx, Info)
        ...
        if Info.Callback then 
            ColorPicker.Callback = Info.Callback
        end 
        ...
        local inputBegan = SatVibMap.InputBegan:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 then
                while InputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
        ...
        local inputBegan2 = HueSelectorInner.InputBegan:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 then
                while InputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
        ...
        addSignal(inputBegan2)

        local inputBegan3 = DisplayFrame.InputBegan:Connect(function(Input)
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        for Idx, Mode in next, Modes do
            ...
            local inputBegan = Label.InputBegan:Connect(function(Input)
                if Input.UserInputType == Enum.UserInputType.MouseButton1 then
                    ModeButton:Select();
                    Library:AttemptSave();
                end;
        ...
        local inputBegan = PickOuter.InputBegan:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 and not Library:MouseIsOverOpenedFrame() then
                Picking = true;
...
do
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        local inputBegan = ToggleRegion.InputBegan:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 and not Library:MouseIsOverOpenedFrame() then
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        local inputBegan = SliderInner.InputBegan:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 and not Library:MouseIsOverOpenedFrame() then
                local mPos = Mouse.X;
